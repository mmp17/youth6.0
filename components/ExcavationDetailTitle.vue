  ...
  <div class="suggest-detail-title">
    <div class="pc">
      <div class="wrapper">
        ...
        <div class="desc">
          <h3>{{ excavation.pro_sub || '' }}</h3>
          <h4>{{ endDate }}</h4>
          <h5>작성자 : {{ excavation.pro_user_name }} &nbsp;&nbsp;||&nbsp;&nbsp; 작성일 : {{ excavation.regdate }}</h5>
        </div>
      </div>
      <div class="likes-views">
        <div class="likes">
          <img src="@/assets/images/icon_like.png" alt="">
          <span>{{ excavation.agreeCount }}</span>
        </div>
        <div class="views">
          <img src="@/assets/images/icon_see.png" alt="">
          <span>{{ excavation.pro_hit }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="mobile">
      ...
      <h3>{{ excavation.pro_sub || '' }}</h3>
      <h4>{{ endDate }}</h4>
      <h5>작성자 : {{ excavation.pro_user_name }}</h5>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    endDate () {
      try {
        return `~${this.excavation.end_date} 까지`
      } catch (e) {
        console.error(e)
        return ''
      }
    ...
    type () {
      return this.excavation.pro_cate || '전체'
    },
    category () {
      const map = {
